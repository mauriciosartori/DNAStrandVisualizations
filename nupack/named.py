...
def complex_analysis(complexes, model, *, compute, options=None):
    ...
    for c in complexes:
        ...
        if v is None:
            continue
        sym = c.symmetry() # have to fix up all this distinguishability stuff
        replace = {}
        
        if v.sample is not None: # take samples out in case of indistinguisable complexes
            replace['sample'] = [v.sample.pop() for i in range(options.num_sample)]
