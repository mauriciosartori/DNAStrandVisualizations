...
class Options(NamedTuple):
    ...
    max_time: float = 0.0 # max time in seconds. 0 if no max time.
...
class Design(Loadable):
    ...
    def __init__(self, tubes, model, *, hard_constraints=(),
    ...
    def run_one(self, restart=None, *, checkpoint_condition=None, checkpoint_handler=None):
        ...
        try:
            raw = self.spec(restart=restart,
                ...
                checkpoint_handler=checkpoint_handler, max_time=float(self.options.max_time))
    ...
    def evaluate(self, env=None):
        ...
        for d in self.domains:
            if d.name.endswith('*'):
                d = (~d).reverse_complement(self.options.wobble_mutations)
    ...
    def build_spec(self):
        ...
        spec = Specification(self.model, wobble_mutations=self.options.wobble_mutations)
        ...
        for d in self.domains:
            s = str((~d).reverse_complement(self.options.wobble_mutations) if d.name.endswith('*') else d)
